...
class TapToRevealCard extends StatefulWidget {
  ...
  const TapToRevealCard({Key? key, required this.step, required this.onReveal})
    : super(key: key);
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          ...
          return Transform(
            ...
            child: isUnder ? _buildRevealedContent() : _buildHiddenContent(),
          );
        },
      ),
    );
    ...
    return AnimatedBuilder(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_isCorrect == false &&
              widget.step.interactionData?['hint'] != null)
            Padding(
...
class TTSControlBar extends StatelessWidget {
  ...
  const TTSControlBar({Key? key, required this.controller}) : super(key: key);
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor: AlwaysStoppedAnimation(_getLevelColor(currentLevel)),
            ),
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            Icon(
              Icons.emoji_events,
              size: 48,
              color: badge.isEarned ? _getTierColor(badge.tier) : Colors.grey,
            ),
            const SizedBox(height: 12),
            Text(
              badge.title,
...
class InteractiveLessonScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Obx(
          () => Text(controller.currentLesson.value?.lessonTitle ?? 'Lesson'),
        ),
        actions: [
          IconButton(
      ...
      body: Obx(() {
        ...
        return Column(
          children: [
            // Progress indicator
            LinearProgressIndicator(value: controller.lessonProgress.value),
  ...
  Widget _buildInteractiveContent(InteractiveStep step) {
    switch (step.interactionType) {
      case InteractionType.tapToReveal:
        return TapToRevealCard(step: step, onReveal: controller.revealStep);
            ...
            () => ElevatedButton.icon(
              ...
              label: Text(
                controller.currentStepIndex.value <
                        controller.lessonSteps.length - 1
                    ? 'Next'
                    : 'Complete',
              ),
            ),
