...
class PracticeController extends GetxController {
  ...
  void _generateQuestions() {
    final random = Random();
    final operators = ['+', '−', '×', '÷'];
    final operator = operators[selectedOperation];
    
    for (int i = 0; i < numberOfQuestions; i++) {
      int num1, num2, answer;
      
      // Progressive difficulty: easy -> medium -> hard
      int difficulty = (i ~/ 7).clamp(0, 2); // 0-6: easy, 7-13: medium, 14-19: hard
  ...
  void _startTotalTimer() {
    ...
    _totalTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_practiceStartTime != null) {
        totalTimeElapsed.value = DateTime.now().difference(_practiceStartTime!).inSeconds;
        
        // Check if total time limit exceeded
        if (totalTimeLimit.value > 0 && totalTimeElapsed.value >= totalTimeLimit.value) {
  ...
  void onNumberTap(String number) {
    if (userInput.value.length < 4) { // Max 4 digits
      ...
      if (userInput.value.length >= expectedLength && userInput.value.length >= 1) {
  ...
  void onSubmit() {
    ...
    // Store user's answer
    questions[currentQuestionIndex.value].userAnswer = int.tryParse(userInput.value);
  ...
  void _endPractice() {
    ...
    Get.offNamed(
      ...
      arguments: {
        'questions': questions,
        'totalTime': totalTimeElapsed.value,
      },
    );
  }
  ...
  int get questionsAnswered => questions.where((q) => q.userAnswer != null).length;
  ...
  String get formattedTime {
    ...
    final remainingTime = totalTimeLimit.value > 0 
        ? (totalTimeLimit.value - totalTimeElapsed.value).clamp(0, totalTimeLimit.value)
  ...
  double get totalTimeProgress {
    ...
    return (totalTimeLimit.value - totalTimeElapsed.value).toDouble() / totalTimeLimit.value.toDouble();
  }
}
