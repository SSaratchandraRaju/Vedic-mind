...
class LessonStepsView extends GetView<VedicCourseController> {
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  _extractCalculation(stepDisplay ?? ''),
                  style: const TextStyle(
                    fontSize: 28,
                    fontWeight: FontWeight.bold,
                    color: AppColors.primary,
  ...
  Widget _buildAllStepsView(Example example) {
    ...
    if (example.step1 != null) {
      ...
      allSteps.add(
        _buildCompactStepCard(stepCounter++, example.step1!, example),
      );
    }
    ...
    if (example.step2 != null) {
      allSteps.add(const SizedBox(height: 16));
      allSteps.add(
        _buildCompactStepCard(stepCounter++, example.step2!, example),
      );
    }
    ...
    if (example.step3 != null) {
      allSteps.add(const SizedBox(height: 16));
      allSteps.add(
        _buildCompactStepCard(stepCounter++, example.step3!, example),
      );
    }
    ...
    if (example.step4 != null) {
      allSteps.add(const SizedBox(height: 16));
      allSteps.add(
        _buildCompactStepCard(stepCounter++, example.step4!, example),
      );
    }
        ...
        children: [
          Row(
            ...
            children: [
              ...
              const SizedBox(width: 12),
              Expanded(
                child: Text(stepContent, style: AppTextStyles.bodyMedium),
              ),
            ],
          ),
          ...
          if (_hasBreakdown(example, stepNumber)) ...[
            const SizedBox(height: 16),
            Center(child: _buildCalculationBreakdown(example, stepNumber)),
          ],
        ],
      ),
    );
  ...
  // Helper function to check if answer is numeric
  bool _isNumericAnswer(String answer) {
    // Remove common symbols and check if what remains is numeric
    final cleaned = answer.trim().replaceAll(RegExp(r'[+\-×÷=\s]'), '');
    return double.tryParse(cleaned) != null;
  }

  // Helper function to extract calculation/equation from step text
  String _extractCalculation(String stepText) {
    // Look for patterns like: "text: calculation" or "text. calculation"
    // Extract only the calculation part (numbers, operators, equations)

    // Pattern 1: Text followed by colon and calculation
    ...
    // Pattern 2: Extract content after common phrases
    final phrases = [
      'Calculate:',
      'Result:',
      'Answer:',
    ...
    for (final phrase in phrases) {
      if (stepText.contains(phrase)) {
        return stepText.split(phrase).last.trim();
      }
    }

    // Pattern 3: Look for equations (contains =, numbers, and operators)
    final equationPattern = RegExp(r'[0-9+\-×÷=().]+');
    final matches = equationPattern.allMatches(stepText);
    if (matches.isNotEmpty) {
      // Find the longest match (likely the main calculation)
      String longestMatch = '';
      ...
      if (longestMatch.length > 3) {
        // At least "1+2" or similar
        return longestMatch;
      }
    }

    // If no calculation found, return the whole text
    return stepText;
  }
}
